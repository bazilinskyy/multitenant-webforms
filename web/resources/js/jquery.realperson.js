/* NOTE FOR MHG USE!
   This plugin was modified. If updates are availible, check the code before
   updating with new files. Main modifications are in generateHTML.
   If questions arise contact <Pavlo Bazilinskyy : pavlo.bazilinskyy@gmail.com>.
...
//                regenerate: '<h:outputText value="#{ui.reloadImage}"/>', // Instruction text to regenerate
...
//			'" value="' + this._hash(text) + '">';
                html += '</div></div><input type="hidden" class="realperson-hash" name="' +
			inst.settings.hashName.replace(/\{n\}/, target.attr('name')) +
			'" value="' + this._hash(text) + '">';
		return html;
	},
...
// Custom div for reloading
$('.realperson-regen-custom').live('click', function() {
	$('.realperson-challenge').next().next().realperson('change');
});
